...
**Stack**: PHP 7.4+, SQLite, JavaScript Vanilla, Efi Bank SDK (preparado)

**Funcionalidades**: Autenticacao, Carrinho, Pagamentos (estrutura preparada), Chat, i18n, Temas

**NOTA**: Sistema de pagamentos Efí Bank está estruturado mas não implementado. Pedidos são criados sem processamento de pagamento.
...
│   ├── services/        # AuthService, CartService, EfiPaymentService (preparado), ExportService, 
│   │                    # NotificationService, PaymentServiceFactory (preparado)
│   └── views/           # home.php, auth/, cart/, payment/ (preparado), restaurant/, admin/, layout/
...
└── public/              # index.php, css/, js/, webhook/ (preparado)
```
...
├── payment_method (preparado - não usado)
├── payment_id (preparado - não usado)
├── payment_status (preparado - não usado)
├── created_at
└── updated_at
...
- **checkout()**: Cria pedido e redireciona (pagamento não implementado)

### PaymentController (ESTRUTURA PREPARADA - NÃO IMPLEMENTADO)
- **selectMethod()**: Exibe opcoes de pagamento (preparado)
- **processPix()**: Gera cobranca PIX (preparado)
- **processCreditCard()**: Processa pagamento com cartao (preparado)
- **generateBoleto()**: Gera boleto bancario (preparado)
- **generateCarne()**: Gera carne parcelado (preparado)
...
-> Pedido criado com status 'pending' (SEM PAGAMENTO)
```

### 5. Acompanhamento
```
Usuario -> /restaurant/orders -> Lista pedidos (sem processamento de pagamento)
```

### FLUXOS PREPARADOS (NÃO IMPLEMENTADOS):

#### Selecao de Pagamento (Preparado)
```
...
## SISTEMA DE PAGAMENTOS (EFI BANK) - STATUS: PREPARADO MAS NÃO IMPLEMENTADO

**IMPORTANTE**: O sistema possui toda a estrutura preparada para integração com Efí Bank, incluindo:
- Controllers de pagamento
- Services para PIX, Cartão, Boleto e Carnê
- Views de seleção e processamento
- Configurações e certificados
- Webhook para recebimento de notificações

**PORÉM**: A integração não está ativa. Pedidos são criados sem processamento de pagamento real.
